...
function HeaderInner({ children, logoLightUrl, logoLightAlt, logoDarkUrl, logoDarkAlt }: HeaderWrapperProps) {
    ...
    return (
        <header
            className={`w-full border-b fixed top-0 left-0 right-0 z-50 pb-1.25 pt-1.25 transition-all duration-300 ${
                scrolled
                    ? "bg-white border-gray-100 shadow-sm [&_nav_a]:text-gray-600 [&_nav_a]:hover:text-blue-600"
                    : "bg-transparent border-transparent [&_nav_a]:!text-white [&_nav_a]:hover:!text-white/80 [&_nav]:bg-white/10 [&_nav]:backdrop-blur-md [&_nav]:rounded-full [&_nav]:px-10 [&_nav]:py-[14.5px] [&_nav_a]:!py-0"
            }`}
        >
